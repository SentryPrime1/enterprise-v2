    ...
    <script>
        ...
        function displayScanResults(result) {
            ...
            console.log('📊 Scan result structure:', {
                hasViolations: !!result.violations,
                violationsLength: result.violations?.length || 0,
                hasPages: !!result.pages,
                pagesLength: result.pages?.length || 0,
                totalViolations: violations.length
            });
            
            // Store violations globally for detailed report
            currentViolations = violations;
            console.log('🔍 Stored violations for detailed report:', currentViolations.length);
        ...
        function openDetailedReport(violations) {
            ...
            let violationsToShow = violations || currentViolations;
            
            // Fallback: if still no violations, try to get from last scan result
            if (!violationsToShow || violationsToShow.length === 0) {
                console.log('⚠️ No violations found, checking for recent scan data...');
                // Try to get violations from the most recent scan display
                const scanResults = document.querySelector('.scan-results');
                if (scanResults) {
                    // This is a fallback - we'll enhance this
                    violationsToShow = currentViolations || [];
                }
            }
            
            console.log('📄 Opening detailed report with violations:', violationsToShow.length);
            console.log('📄 Sample violations:', violationsToShow.slice(0, 3).map(v => ({ id: v.id, impact: v.impact })));
